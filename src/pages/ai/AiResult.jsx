...
const AiResult = () => {
  ...
  const handleClose = () => {
    if (showAsPopup) {
      ...
      // 애니메이션 완료 후 홈화면으로 이동
      setTimeout(() => {
        // localStorage 우선 사용, 없으면 AI 결과 페이지의 centerLat/centerLng 사용
        const savedLocation = localStorage.getItem('lastMapLocation');
        let returnLocation = null;
        
        if (savedLocation) {
          returnLocation = JSON.parse(savedLocation);
          console.log('📍 AI 결과 페이지 닫기 (팝업) - localStorage에서 가져온 위치와 줌:', returnLocation);
        } else {
          returnLocation = {
            lat: centerLat,
            lng: centerLng,
          ...
          console.log('📍 AI 결과 페이지 닫기 (팝업) - centerLat/centerLng 사용:', returnLocation);
        }
      ...
      if (savedLocation) {
        returnLocation = JSON.parse(savedLocation);
        console.log('📍 AI 결과 페이지 닫기 - localStorage에서 가져온 위치와 줌:', returnLocation);
      } else {
        returnLocation = {
          lat: centerLat,
          lng: centerLng,
        ...
        console.log('📍 AI 결과 페이지 닫기 - centerLat/centerLng 사용:', returnLocation);
      }
      ...
      <div className="relative">
        <Header
          ...
          onBack={() => {
            ...
            if (savedLocation) {
              returnLocation = JSON.parse(savedLocation);
              console.log('📍 AI 결과 페이지 헤더 뒤로가기 - localStorage에서 가져온 위치와 줌:', returnLocation);
            } else {
              returnLocation = {
                lat: centerLat,
                lng: centerLng,
              ...
              console.log('📍 AI 결과 페이지 헤더 뒤로가기 - centerLat/centerLng 사용:', returnLocation);
            }
